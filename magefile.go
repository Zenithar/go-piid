...
func Build() {
	banner := figure.NewFigure("PIId", "", true)
	banner.Print()
...
func (Bin) PIId() error {
	return goBuild("go.zenithar.org/piid/cli/piid", "piid")
}
...
func goBuild(packageName, out string) error {
	...
	varsSetByLinker := map[string]string{
		"go.zenithar.org/piid/internal/version.Version":   tag(),
		"go.zenithar.org/piid/internal/version.Revision":  hash(),
		"go.zenithar.org/piid/internal/version.Branch":    branch(),
		"go.zenithar.org/piid/internal/version.BuildUser": os.Getenv("USER"),
		"go.zenithar.org/piid/internal/version.BuildDate": time.Now().Format(time.RFC3339),
		"go.zenithar.org/piid/internal/version.GoVersion": runtime.Version(),
	}
